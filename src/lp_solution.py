...
class Problem:
    ...
    def __init__(self, N, M, bounds, edges):
        ...
        self.objective = cp.Minimize(
            cp.sum([(self.edges[i][2] * self.x[i]) for i, _ in enumerate(self.edges)])
        )
...
def __powerset(vertices):
    ...
    """
    return chain.from_iterable(combinations(input, r) for r in range(len(vertices) + 1))
...
def __solve_lp(problem, edges, E, F, W, V):
    ...
    (
        problem.constraints.append(
            cp.sum(cp.vstack(problem.x)) == (problem.num_nodes - 1)
        )
    )
        ...
        if not changed:
            ...
            for (u, v, dist) in E:
                ...
                if abs(minus_one) < min_dist:
                    if not (
                        __num_edges_containing(F, u) == problem.node_bounds[u][0]
                    ) or (__num_edges_containing(F, v) == problem.node_bounds[v][0]):
